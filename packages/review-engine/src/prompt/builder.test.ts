...
+  const label = "urgent";
    ...
    expect(prompts).toMatchInlineSnapshot(`
      {
        ...
        "strict": "You are Sovri's review engine. Review only the supplied pull request metadata and unified diff. Hold the diff to a high bar: report every valid blocker, major, and minor compliance-relevant issue (bug, security, or regulatory compliance) that justifies at least minor severity. Never describe what the code does; a hunk with no issue yields no finding. Each finding states the problem and its impact in \`body\` and the concrete fix in \`recommendation\`. Write a neutral one-paragraph \`summary\` separately from the findings. Return structured JSON findings that match the requested schema. On every security, bug, or compliance finding tied to a known weakness, set \`cwe\` to its CWE id (for example CWE-89) and \`confidence\` to a number between 0 and 1 reflecting your honest certainty; omit \`cwe\` when the finding has no associated weakness. A resolved \`cwe\` maps the finding to GDPR, DORA, AI Act, and NIS2 references, so a missing one drops that compliance context.",
      }
    `);
  });
  ...
  it("emits comprehensive guidance for strict mode", () => {
    ...
    // And the prompt names the compliance scope.
